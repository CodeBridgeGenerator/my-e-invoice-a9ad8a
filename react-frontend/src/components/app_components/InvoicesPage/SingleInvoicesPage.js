    ...
    useEffect(() => {
        ...
        client
            ...
            .get(urlParams.singleInvoicesId, { query: { $populate: [            {
                ...
                service: "users",
                select: ["name"],
              },"invoiceType","classification","countryOfOrigin","taxType1","buyer","supplier"] }})
            .then((res) => {
                set_entity(res || {});
...
const classification = Array.isArray(res.classification)
            ? res.classification.map((elem) => ({ _id: elem._id, classificationCode: elem.classificationCode }))
            : res.classification
                ? [{ _id: res.classification._id, classificationCode: res.classification.classificationCode }]
                : [];
...
const buyer = Array.isArray(res.buyer)
            ? res.buyer.map((elem) => ({ _id: elem._id, buyersName: elem.buyersName }))
            : res.buyer
                ? [{ _id: res.buyer._id, buyersName: res.buyer.buyersName }]
                : [];
        setBuyer(buyer);
const supplier = Array.isArray(res.supplier)
            ? res.supplier.map((elem) => ({ _id: elem._id, suppliersName: elem.suppliersName }))
            : res.supplier
                ? [{ _id: res.supplier._id, suppliersName: res.supplier.suppliersName }]
                : [];
  ...
  const copyPageLink = () => {
    ...
    return (
        ...
        <div className="col-12 flex flex-column align-items-center">
            ...
            <div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Invoice Date and Time</label><p id="invoiceDateAndTime" className="m-0 ml-3" >{_entity?.invoiceDateAndTime}</p></div>
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Tax Rate (%)</label><p className="m-0 ml-3" >{_entity?.taxRate}</p></div>
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Tax Amount</label><p className="m-0 ml-3" >{_entity?.taxAmount}</p></div>
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Amount Exempted</label><p className="m-0 ml-3" >{_entity?.amountExempted}</p></div>
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Discount Rate (%)</label><p className="m-0 ml-3" >{_entity?.discountRate}</p></div>
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Discount Amount</label><p className="m-0 ml-3" >{_entity?.discountAmount}</p></div>
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Fee/Charge Rate (%)</label><p className="m-0 ml-3" >{_entity?.feeChargeRate}</p></div>
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Fee/Charge Amount</label><p className="m-0 ml-3" >{_entity?.feeChargeAmount}</p></div>
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Total Taxable Amount per Tax Type</label><p className="m-0 ml-3" >{_entity?.totalTaxableAmountPerTaxType}</p></div>
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Total Tax Amount Per Tax Type</label><p className="m-0 ml-3" >{_entity?.totalTaxAmountPerTaxType}</p></div>
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Classification</label>
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Country of Origin</label>
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Buyer</label>
                    {buyer.map((elem) => (
                        <Link key={elem._id} to={`/buyers/${elem._id}`}>
                        <div>
                  {" "}
                            <p className="text-xl text-primary">{elem.buyersName}</p>
                            </div>
                        </Link>
                    ))}</div>
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Supplier</label>
                    {supplier.map((elem) => (
                        ...
                        <div>
                  {" "}
                            <p className="text-xl text-primary">{elem.suppliersName}</p>
                            </div>
                        </Link>
                    ))}</div>
