...
const [supplier, setSupplier] = useState([])
 ...
 useEffect(() => {
                    ...
                    client
                        .service("buyers")
                        .find({ query: { $limit: 10000, $sort: { createdAt: -1 }, _id : urlParams.singleBuyersId } })
                        .then((res) => {
                            setBuyer(res.data.map((e) => { return { name: e['buyersName'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.log({ error });
 ...
 useEffect(() => {
                    ...
                    client
                        ...
                        .then((res) => {
                            setSupplier(res.data.map((e) => { return { name: e['suppliersName'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.log({ error });
...
no: _entity?.no,
classification: _entity?.classification?._id,
...
buyer: _entity?.buyer?._id,
supplier: _entity?.supplier?._id,
        };
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Calendar id="invoiceDateAndTime" value={_entity?.invoiceDateAndTime ? new Date(_entity?.invoiceDateAndTime) : null} onChange={ (e) => setValByKey("invoiceDateAndTime", new Date(e.target.value))} showIcon showButtonBar  />
            </span>
            <small className="p-error">
