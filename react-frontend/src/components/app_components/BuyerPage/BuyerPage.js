...
const BuyerPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("buyer");
    ...
    useEffect(() => {
        ...
        client
            .service("buyer")
            .find({ query: { $limit: 10000 , identifierType : urlParams.singleIdentifyTypeId ,buyersAddressCountryName : urlParams.singleCountryCodeId ,buyersAddressStateName : urlParams.singleStateCodeId ,theFirstBuyersContactNumber : urlParams.singlePhoneNumberPrefixId ,invoiceCurrency : urlParams.singleCurrencyCodeId ,frequencyOfBilling : urlParams.singleFrequencyOfBillingId ,paymentMode : urlParams.singlePaymentModeId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("buyer").remove(selectedEntityIndex);
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("buyer").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="buyer-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="buyer-datatable">
                    <BuyerDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <BuyerEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <BuyerCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
