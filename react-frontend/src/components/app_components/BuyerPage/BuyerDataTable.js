...
const BuyerDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.buyersName}</p>
...
const dropdownTemplate17 = (rowData, { rowIndex }) => <p >{rowData.paymentMode?.paymentMode}</p>
  ...
  const handleHideDialog = () => {
    ...
    return (
        ...
        />
<Column field="buyersName" header="Buyer's name (*)" body={pTemplate0} filter={selectedFilterFields.includes("buyersName")} hidden={selectedHideFields?.includes("buyersName")}  sortable style={{ minWidth: "8rem" }} />
...
<Column field="paymentMode" header="Payment Mode" body={dropdownTemplate17} filter={selectedFilterFields.includes("paymentMode")} hidden={selectedHideFields?.includes("paymentMode")}  style={{ minWidth: "8rem" }} />
      ...
      {selectedItems.length > 0 ? (
        ...
        <Dialog header="Upload Buyer Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="buyer"            
          onUploadComplete={() => {
      ...
      <Dialog header="Search Buyer" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
