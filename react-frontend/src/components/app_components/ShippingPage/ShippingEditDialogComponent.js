...
referenceNumberOfCustomsFormNo19Etc: _entity?.referenceNumberOfCustomsFormNo19Etc,
incoterms: _entity?.incoterms,
freeTradeAgreementFtaInformation: _entity?.freeTradeAgreementFtaInformation,
...
referenceNumberOfCustomsFormNo2: _entity?.referenceNumberOfCustomsFormNo2,
        ...
        await client.service("shipping").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info shipping updated successfully" });
...
const shippingRecipientsIdentifierTypeOptions = shippingRecipientsIdentifierType.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Edit Shipping" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="shipping-edit-dialog-component">
...
(Postal Zone):</label>
                <InputNumber id="shippingRecipientsAddressPostalZone" className="w-full mb-3 p-inputtext-sm" value={_entity?.shippingRecipientsAddressPostalZone} onChange={(e) => setValByKey("shippingRecipientsAddressPostalZone", e.value)}  useGrouping={false}/>
            </span>
            <small className="p-error">
