...
import shippingFakerFactory from "./shippingFakerFactory";
...
const ShippingFakerDialogComponent = (props) => {
...
client.service("identifyType")
    ...
    const onRun = async () => {
        let fakeData = shippingFakerFactory(props.user,fakerCount,shippingRecipientsAddressCountryNameIds,shippingRecipientsAddressStateNameIds,shippingRecipientsIdentifierTypeIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("shipping").create(elem));
...
export default connect(mapState, mapDispatch)(ShippingFakerDialogComponent);
