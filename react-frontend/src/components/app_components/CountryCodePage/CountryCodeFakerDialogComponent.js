...
import countryCodeFakerFactory from "./countryCodeFakerFactory";
...
const CountryCodeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = countryCodeFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("countryCode").create(elem));
...
export default connect(mapState, mapDispatch)(CountryCodeFakerDialogComponent);
