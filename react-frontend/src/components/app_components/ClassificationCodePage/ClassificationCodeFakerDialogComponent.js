...
import classificationCodeFakerFactory from "./classificationCodeFakerFactory";
...
const ClassificationCodeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = classificationCodeFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("classificationCode").create(elem));
...
export default connect(mapState, mapDispatch)(ClassificationCodeFakerDialogComponent);
