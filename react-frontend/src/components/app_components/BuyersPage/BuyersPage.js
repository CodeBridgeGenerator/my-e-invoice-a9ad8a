...
import BuyersDatatable from "./BuyersDataTable";
...
import BuyersFakerDialogComponent from "./BuyersFakerDialogComponent";
import BuyersSeederDialogComponent from "./BuyersSeederDialogComponent";
...
const BuyersPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("buyers");
    ...
    useEffect(() => {
        ...
        client
            .service("buyers")
            .find({ query: { $limit: 10000 , identifierType : urlParams.singleIdentifyTypeId ,buyersAddressCountryName : urlParams.singleCountryCodesId ,buyersAddressStateName : urlParams.singleStateCodesId ,theFirstBuyersContactNumber : urlParams.singlePhoneNumberPrefixId ,invoiceCurrency : urlParams.singleCurrencyCodesId ,frequencyOfBilling : urlParams.singleFrequencyOfBillingId ,paymentMode : urlParams.singlePaymentModesId ,team : urlParams.singleTeamId ,invoiceId : urlParams.singleInvoicesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("buyers").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Buyers", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("buyers").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="buyers-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="buyers-datatable">
                    <BuyersDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <BuyersEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <BuyersCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <BuyersFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <BuyersSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(BuyersPage);
