...
const [paymentMode, setPaymentMode] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [identifierType,buyersAddressCountryName,buyersAddressStateName,theFirstBuyersContactNumber,invoiceCurrency,frequencyOfBilling,paymentMode], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            buyersName: _entity?.buyersName,buyersTin: _entity?.buyersTin,buyersSstRegistrationNumber: _entity?.buyersSstRegistrationNumber,identifierType: _entity?.identifierType?._id,businessRegistrationNumberIdentificationNumberPassportNumber: _entity?.businessRegistrationNumberIdentificationNumberPassportNumber,buyersEMail: _entity?.buyersEMail,buyersAddressCountryName: _entity?.buyersAddressCountryName?._id,buyersAddressStateName: _entity?.buyersAddressStateName?._id,buyersAddressCityName: _entity?.buyersAddressCityName,buyersAddressPostalZone: _entity?.buyersAddressPostalZone,theFirstBuyersContactNumber: _entity?.theFirstBuyersContactNumber?._id,buyersContactNumber: _entity?.buyersContactNumber,invoiceCurrency: _entity?.invoiceCurrency?._id,currencyExchangeRate: _entity?.currencyExchangeRate,frequencyOfBilling: _entity?.frequencyOfBilling?._id,billingPeriodStartDate: _entity?.billingPeriodStartDate,billingPeriodEndDate: _entity?.billingPeriodEndDate,paymentMode: _entity?.paymentMode?._id,
        ...
        const result = await client.service("buyers").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Buyers updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Buyers" });
        }
        setLoading(false);
    };
...
useEffect(() => {
                    ...
                    client
                        ...
                        .then((res) => {
                            setBuyersAddressCountryName(res.data.map((e) => { return { name: e['countryCode'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.log({ error });
...
useEffect(() => {
                    ...
                    client
                        ...
                        .then((res) => {
                            setBuyersAddressStateName(res.data.map((e) => { return { name: e['stateCode'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.log({ error });
...
useEffect(() => {
                    ...
                    client
                        ...
                        .then((res) => {
                            setTheFirstBuyersContactNumber(res.data.map((e) => { return { name: e['phoneNumberPrefix'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.log({ error });
...
useEffect(() => {
                    ...
                    client
                        .service("currencyCodes")
                        .find({ query: { $limit: 10000, $sort: { createdAt: -1 }, _id : urlParams.singleCurrencyCodesId } })
                        .then((res) => {
                            setInvoiceCurrency(res.data.map((e) => { return { name: e['currencyCode'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.log({ error });
                            props.alert({ title: "CurrencyCodes", type: "error", message: error.message || "Failed get currencyCodes" });
                        });
                }, []);
...
useEffect(() => {
                    ...
                    client
                        .service("paymentModes")
                        .find({ query: { $limit: 10000, $sort: { createdAt: -1 }, _id : urlParams.singlePaymentModesId } })
                        .then((res) => {
                            setPaymentMode(res.data.map((e) => { return { name: e['paymentMode'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.log({ error });
                            props.alert({ title: "PaymentModes", type: "error", message: error.message || "Failed get paymentModes" });
                        });
                }, []);
...
const paymentModeOptions = paymentMode.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create Buyers" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="buyers-create-dialog-component">
            <div className="col-12 md:col-6 field">
            <span className="align-items-center">
                <label htmlFor="buyersName">Buyer's name (*):</label>
                <InputText id="buyersName" className="w-full mb-3 p-inputtext-sm" value={_entity?.buyersName} onChange={(e) => setValByKey("buyersName", e.target.value)}  />
            </span>
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="buyersContactNumber" className="w-full mb-3 p-inputtext-sm" value={_entity?.buyersContactNumber} onChange={(e) => setValByKey("buyersContactNumber", e.value)}  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Dropdown id="invoiceCurrency" value={_entity?.invoiceCurrency?._id} optionLabel="name" optionValue="value" options={invoiceCurrencyOptions} onChange={(e) => setValByKey("invoiceCurrency", {_id : e.value})}  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Calendar id="billingPeriodStartDate"  value={_entity?.billingPeriodStartDate ? new Date(_entity?.billingPeriodStartDate) : null} dateFormat="dd/mm/yy" onChange={ (e) => setValByKey("billingPeriodStartDate", new Date(e.target.value))} showIcon showButtonBar  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Calendar id="billingPeriodEndDate"  value={_entity?.billingPeriodEndDate ? new Date(_entity?.billingPeriodEndDate) : null} dateFormat="dd/mm/yy" onChange={ (e) => setValByKey("billingPeriodEndDate", new Date(e.target.value))} showIcon showButtonBar  />
            </span>
            <small className="p-error">
