...
import taxTypeFakerFactory from "./taxTypeFakerFactory";
...
const TaxTypeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = taxTypeFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("taxType").create(elem));
...
export default connect(mapState, mapDispatch)(TaxTypeFakerDialogComponent);
