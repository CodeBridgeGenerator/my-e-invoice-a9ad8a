...
import stateCodeFakerFactory from "./stateCodeFakerFactory";
...
const StateCodeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = stateCodeFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("stateCode").create(elem));
...
export default connect(mapState, mapDispatch)(StateCodeFakerDialogComponent);
