...
import currencyCodeFakerFactory from "./currencyCodeFakerFactory";
...
const CurrencyCodeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = currencyCodeFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("currencyCode").create(elem));
...
export default connect(mapState, mapDispatch)(CurrencyCodeFakerDialogComponent);
