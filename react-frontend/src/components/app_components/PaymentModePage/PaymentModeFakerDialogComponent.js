...
import paymentModeFakerFactory from "./paymentModeFakerFactory";
...
const PaymentModeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = paymentModeFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("paymentMode").create(elem));
...
export default connect(mapState, mapDispatch)(PaymentModeFakerDialogComponent);
