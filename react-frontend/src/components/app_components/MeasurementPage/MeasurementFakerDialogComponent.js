...
import measurementFakerFactory from "./measurementFakerFactory";
...
const MeasurementFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = measurementFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("measurement").create(elem));
...
export default connect(mapState, mapDispatch)(MeasurementFakerDialogComponent);
